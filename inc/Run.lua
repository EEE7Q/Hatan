...
SUDO_USER = redis:hgetall(max..'username:'..SUDO_ID).username
sendPhoto(msg.chat_id_,msg.id_,redis:get(max..':WELCOME_BOT'),[[-› اهلاً وسهلاً انا بوت اسمي]]..redis:get(max..':NameBot:')..[[ ⚜
-› اختصـاصـي حمـاية المـجمـوعات
-› مـن السـبام والتوجيه والتكرار والخ...
-› يُورز المطور  : ]]..SUDO_USER:gsub([[\_]],'_')..[[ .
]])
return false
end
if not msg then
...
send_msg(msg.chat_id_,"-› تم تثبيت الايدي الان قم برسال معرف المطور @UserName ...")
	...
	if msg.text== 'تغيير المطور الاساسي' and msg.sender_user_id_ == SUDO_ID then
